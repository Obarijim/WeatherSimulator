namespace WeatherSimulator
{
    internal class Program
    {
        static void Main(string[] args)
        {
            Console.WriteLine("Enter the number of days you want to simulate");
            int days = int.Parse(Console.ReadLine());

            int[] temperature = new int[days];
            string[] conditions = { "Sunny", "Rainy", "Cloudy", "Rainy" };
            string[] weatherConditions = new string[days];

            Random random = new Random();

            for (int i = 0; i < days; i++)
            {
                temperature[i] = random.Next(-15, 30);
                weatherConditions[i] = conditions[random.Next(conditions.Length)];
            }

            Console.WriteLine($"The maximum temperature was {maxTemp(temperature)}");
            Console.WriteLine($"The maximum temperature was {minTemp(temperature)}");
            Console.WriteLine($"The average temperature is " + tempAverage(temperature));
            Console.WriteLine($"The most common condition was {MostCommonCondition(conditions)}");

            Console.ReadKey();
        }

        static string MostCommonCondition(string[] conditions)
        {
            int count = 0;
            string mostCommon = conditions[0];

            for (int i = 0; i < conditions.Length; i++)
            {
                int tempCount = 0;
                for (int j = 0; j < conditions.Length; j++)
                {
                    if (conditions[j] == conditions[i]) {
                        tempCount++;
                    }
                }
                if (tempCount > count) {
                    count = tempCount;
                    mostCommon = conditions[i];
                        }
            }
            return mostCommon;
        }

        static double tempAverage(int[] temperature)
        {
            double sum = 0;

            for (int i = 0; i < temperature.Length; i++)
            {
                sum += temperature[i];
            }
            double average = sum / temperature.Length;

            return average;
        }

        static int minTemp(int[] temperature)
        {
            int min = temperature[0];
            foreach (int temp in temperature)
            {
                if (temp < min)
                    min = temp;
            }
            return min;
        }

        static int maxTemp(int[] temperature)
        {
            int max = temperature[0];
            foreach (int temp in temperature)
            {
                if (temp > max)
                    max = temp;
            }
            return max;
        }
    }
}